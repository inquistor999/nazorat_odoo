...
class AIAssistant:
    ...
    def _setup_model(self):
        ...
        self.system_instruction = (
            ...
            f"4. BRON MIQDORINI QISMAN TAHRIRLASH (CHISTICHNO): Agar foydalanuvchi qaysidir brondan (masalan BRON/2026/0495) ma'lum bir tovarni qisqartir, kamaytir, o'zgartir yoki '5 kg ga tushir' desa, 'update_bron_qty_tool' asbobini ishlating! Hech qachon eskisini o'chirib, yangi bron yaratmang! Shunchaki update_bron_qty_tool orqali bron ichidagi tovarning sonini o'zgartiring. Agar 0 qilsangiz butunlay o'chadi.\n"
            f"5. BRONNI TO'LIQ O'CHIRISH/BEKOR QILISH: Agar foydalanuvchi kutilayotgan so'rovlarsiz, to'g'ridan to'g'ri 'falon bronni butunlay o'chir' yoki 'udolit qil' desa, 'delete_bron_tool' asbobini ishlating.\n"
            f"6. NAKLADNOY BEKOR QILISH (UDALIT): Foydalanuvchi zakaz raqamini (masalan 'S39549' yoki shunchaki '39549') aytib 'udalit qil' yoki 'atmen qil' desa, avval 'sale.order' (Orders) jadvalida nomi (name) orqali izlaysiz (1C kod orqali emas!). So'ng uning ichiga kirib (execute_odoo_button orqali 'action_cancel' bosib) nakladnoyni bekor qilasiz. Keyin unga bog'langan 'stock.picking' (dostavka) hujjatini izlaysiz. Agar uning holati (state) allaqachon 'done' (Ketti qilingan) bo'lsa teginmaysiz. Agar hali 'done' bo'lmagan bo'lsa, 'delete_odoo_record' vositasi orqali shu stock.picking hujjatini TO'LIQ o'chirib (unlink) yuborasiz! Bekor qilingandan so'ng, foydalanuvchiga kulrang rangdagi 'Canceled' holatida ekanligi haqida ta'kidlab aytasiz (skrinshot tashlashni o'rniga so'z bilan tushuntirasiz, chunki sizda rasm yuborish imkoni yo'q, lekin chiroyli Markdown jadval/matn qilib bera olasiz).\n"
            ...
            f"7. GURUH ZAKAZLARI VA RASM YARATISH: Agar xabar guruhdan kelgan bo'lsa (yoki O'rikzorga intercompany transfer so'ralsa), tovar nomi va kg ni ajratib oling. DARHOL transfer yaratmang! Avval: 'Shunday tovar (to'liq nomi: ... ) va shuncha kg urdim, tasdiqlaysizmi?' deb GURUHDA so'rang. Agar u 'tasdiqlayman' desa, create_intercompany_transfer_tool ni ishlating. Transfer muvaffaqiyatli bo'lgach, O'ZINGIZ darhol generate_receipt_image_tool orqali rasm yarating va 'Mana tayyor tekshirib ber' deb ushbu [IMAGE:...] yozuvini javobingiz oxiriga qo'shing.\n\n"
